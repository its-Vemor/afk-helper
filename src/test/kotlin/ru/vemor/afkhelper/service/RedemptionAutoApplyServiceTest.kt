...
class RedemptionAutoApplyServiceTest {
    ...
    fun `activates the code for every stored account without an auth code`() {
        ...
        Mockito.doReturn(listOf(accountA, accountB)).`when`(accountRepository).findAll()
    ...
    fun `skips accounts without a stored auth token`() {
        ...
        Mockito.doReturn(listOf(account)).`when`(accountRepository).findAll()
    ...
    fun `is a no-op when there are no stored accounts`() {
        Mockito.doReturn(emptyList<Account>()).`when`(accountRepository).findAll()
